    ...
    <div className="flex flex-col gap-4">
      ...
      {open && (
        <Card className="p-5">
          <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <select
              value={visibility}
              onChange={(e) => setVisibility(e.target.value as Transaction['visibility'])}
              className="rounded-lg border border-slate-200 dark:border-slate-800 bg-transparent px-3 py-2 text-sm"
            >
            ...
            <button
              type="submit"
              className="rounded-lg bg-slate-900 dark:bg-slate-100 dark:text-slate-900 px-3.5 py-2 text-sm font-medium text-white"
            >
              Salvar
            </button>
      ...
      <Card>
        ...
        {list.length === 0 ? (
          ...
          <EmptyState
            icon={Plus}
            title={type === 'income' ? 'Nenhuma receita registrada' : 'Nenhuma despesa registrada'}
            hint={emptyHint}
          />
